...
export CROSS_COMPILE="/home/vicatz/toolchains/linaro_gcc/gcc-linaro-7.2.1-2017.11-x86_64_aarch64-linux-gnu/bin/aarch64-linux-gnu-"
kernel="wayang"
version="v1.4"
vendor="xiaomi"
android="Miui"
...
if [ -f arch/arm64/boot/"$kerneltype" ]; then
	cp arch/arm64/boot/"$kerneltype" "$zip"/tools/"$kerneltype"
        mkdir -p zip/system/lib/modules/pronto
	cp drivers/staging/prima/wlan.ko zip/system/lib/modules/pronto/pronto_wlan.ko
	find . -name '*.ko' -exec cp {} $modules_dir/ \;
	"$CROSS_COMPILE"strip --strip-unneeded "$zip"/system/lib/modules/*.ko &> /dev/null
        mkdir -p zip/system/lib/modules/pronto/
